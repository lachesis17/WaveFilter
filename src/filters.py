...
from scipy.signal import filtfilt, find_peaks, iirfilter, iirnotch
...
class Filters:
    ...
    def apply_standard_filter(signal_series, sample_rate, filter_type, low_freq, high_freq, order):
        """
        Apply standard IIR filter. Returns filtered np.ndarray or None on error.
        """
        def _filter(data, freq, ord_):
            try:
                if filter_type == "Notch Filter":
                    ...
                    b, a = iirfilter(ord_, Wn=freq, fs=sample_rate, btype='bandpass', ftype='butter')
                elif filter_type == "Low Pass Filter":
                    b, a = iirfilter(ord_, Wn=freq, fs=sample_rate, btype='lowpass', ftype='butter')
                elif filter_type == "High Pass Filter":
                    b, a = iirfilter(ord_, Wn=freq, fs=sample_rate, btype='highpass', ftype='butter')
                else:
                    return None
